...
def create_model_performance_visualizations(model, X_train: pd.DataFrame, y_train: pd.Series, 
                                            X_test: pd.DataFrame, y_test: pd.Series, 
                                            evaluation_results: dict, artifacts_dir: str, model_name: str):
    """Create comprehensive model performance visualizations including Learning Curves."""
    try:
        ...
        # --- 2. Learning Curves (New Addition) ---
        try:
            ...
            lc_path = os.path.join(model_dir, f'learning_curve_{model_name}.png')
            plt.savefig(lc_path, dpi=300, bbox_inches='tight')
            plt.close()
            mlflow.log_artifact(lc_path, f"model_performance/{model_name}")
        ...
        logger.info(f"✓ All performance visualizations (including Learning Curves) created for {model_name}")
...
def training_pipeline(
    ...
    model, X_train, Y_train.squeeze(), X_test, Y_test.squeeze(), 
    evaluation_results, run_artifacts_dir, 'XGboost'
)
